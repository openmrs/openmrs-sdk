...
public class BuildDistro extends AbstractTask {
	...
	private void copyDockerfile(File targetDirectory, DistroProperties distroProperties) throws MojoExecutionException {
		...
		if (majorVersion == 1) {
			...
			String dockerImageTag = distroProperties.getPropertyValue("docker.image.tag");
			if (StringUtils.isBlank(dockerImageTag)) {
				...
				if (StringUtils.isNotBlank(openmrsVersion)) {
					...
					if (namespace.equals("openmrs") && repository.equals("openmrs-core") && openmrsVersion.endsWith("-SNAPSHOT")) {
						String[] parts = openmrsVersion.split("\\.");
			...
			if (StringUtils.isBlank(dockerImageTag)) {
				if (isPlatform2point5AndAbove(platformVersion)) {
					dockerImageTag = "nightly-amazoncorretto-11";
				}
				else {
					dockerImageTag = "nightly-amazoncorretto-8";
				}
			}
