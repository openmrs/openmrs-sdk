...
public class DefaultGitHelper implements GitHelper {
    ...
    @Override
    public void pullRebase(AbstractTask task, String branch, Project project) throws MojoExecutionException {
        Pull pull = new Pull(task, branch);
    ...
    @Override
    public Iterable<PushResult> push (Git git, String username, String password) {
    ...
    public Iterator<RevCommit> getCommitDifferential(Git git, Repository repository, String baseRef, String headRef) {
        Ref localRef = repository.getAllRefs().get(headRef);
        Ref upstreamRef = repository.getAllRefs().get(baseRef);
        try {
            return git.log().addRange(upstreamRef.getObjectId(), localRef.getObjectId()).call().iterator();
