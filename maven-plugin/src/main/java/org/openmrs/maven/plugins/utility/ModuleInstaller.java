...
public class ModuleInstaller {
	...
	public ModuleInstaller(MavenProject mavenProject, MavenSession mavenSession, BuildPluginManager pluginManager,
	...
	public void installModulesForDistro(Server server, DistroProperties properties, DistroHelper distroHelper)
	...
	private void prepareModules(Artifact[] artifacts, String outputDir, String goal) throws MojoExecutionException {
		...
		executeMojo(
		    plugin(groupId(SDKConstants.DEPENDENCY_PLUGIN_GROUP_ID), artifactId(SDKConstants.DEPENDENCY_PLUGIN_ARTIFACT_ID),
		        version(SDKConstants.DEPENDENCY_PLUGIN_VERSION)),
		    goal(goal), configuration(configuration.toArray(new Element[0])),
